...
Hay numerosas referencias válidas en internet para aprender XML y tecnologías asociadas. En esta presentación, vamos a basarnos en las dos siguientes:
...
[XML](http://en.wikipedia.org/wiki/XML) (*eXtensible Mark-up Language*) es un lenguaje de marcas orientado a transmitir y almacenar información de manera estructurada. Se ha convertido en uno de los principales formatos de intercambio de información utilizado en Internet y en bases de datos.

Fue diseñado por el [W3C](http://www.w3c.es) como un subconjunto más restrictivo a partir de otro lenguaje de marcas anterior llamado [SGML](http://en.wikipedia.org/wiki/Standard_Generalized_Markup_Language).

XML es un *metalenguaje*, es decir, un lenguaje (de marcas) que permite definir otros lenguajes (de marcas).
...
- Las versiones de HTML que hemos visto (HTML 4.1) están diseñadas en [SGML](http://en.wikipedia.org/wiki/Standard_Generalized_Markup_Language).
- De manera paralela, existen versiones modernas de HTML que están basadas en XML y que pueden ser documentos XML válidos ([XHTML](http://en.wikipedia.org/wiki/XHTML)).
...
En XML estos elementos no tienen ningún significado (aunque lo ideal es darles nombres inteligiblers que sean lo suficientemente expresivos para intuir su semántica). Lo que hacemos es simplemente definir la relación de un elemento con respecto a otros: 
...
Para más detalles sobre estas palabras reservadas, consulta [el tutorial de DTD en W3Schools](http://www.w3schools.com/dtd/dtd_building.asp).


# Sintaxis de las DTDs: cuantificadores
...
- La interrogación `?` indica que el elemento es opcional, pero si aparece solo lo hace una vez: *0 o 1 vez*.
...
# Sintaxis de las DTDs: conectores
...
# Sintaxis de las DTDs: Atributos
...
# Sintaxis de las DTDs: Entidades
...
Recuerda que [en HTML están definidas algunas entidades](http://ascii.cl/es/codigos-html.htm) que hemos visto y usado:

```html
<p>Las vocales con tilde son &aacute;, &eacute;, &iacute;, &oacute; y &uacute;.</p>
```    
...
[^gentle]:  A Gentle Introduction to XML http://www.tei-c.org/release/doc/tei-p5-doc/en/html/SG.html
[^w3s]: Tutorial de XML en W3Schools http://www.w3schools.com/xml/
[^dtd]: Tutorial de DTDs en W3Schools http://www.w3schools.com/dtd/default.asp
